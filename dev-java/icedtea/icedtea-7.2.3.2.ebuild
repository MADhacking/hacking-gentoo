...
inherit autotools cadb java-pkg-2 java-vm-2 pax-utils prefix versionator virtualx
...
src_install() {
	...
	dosym "/usr/share/java-cert-db/cacerts" "${ddest}/jre/lib/security/cacerts"
...
pkg_postinst() {
	# Create the certificate DB.
	cadb_pkg_postinst java
}

pkg_postrm() {
	# Do nothing, this is needed to prevent the inherited cadb_pkg_postrm going off.
	true
}
