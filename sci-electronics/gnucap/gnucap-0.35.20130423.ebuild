...
src_prepare() {
	# Fix the paths and variables in the models-* Make2 files.
	sed -i -e 's:CFLAGS = -O2 -g:CPPFLAGS +=:' \
		-e '/CCFLAGS =/i\CFLAGS += $(CPPFLAGS)' \
		-e 's:CCFLAGS = $(CFLAGS):CXXFLAGS += $(CPPFLAGS):' \
		-e 's:LDFLAGS = :LDFLAGS += :' \
		-e 's:CCFLAGS:CXXFLAGS:' \
		-e "s:../Gnucap:${S}/include:" \
		models-*/Make2 || die "sed failed"

	# Fix parallel make.
	sed -i -e 's:; make:; $(MAKE):' \
		models-*/Makefile || die "sed failed"

	epatch "${FILESDIR}"/dynamic-linking.patch
	epatch "${FILESDIR}"/readline_test.patch
...
src_install () {
	emake PREFIX="${D}" DESTDIR="${D}" install || die "Installation failed"
	insopts -m0755
	for PLUGIN_DIR in models-* ; do
